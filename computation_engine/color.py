...
COLOR_LOOKUP_TABLE_PATH = './clookup.json'
...
def process_temperatures(temp_array, lookup_table):
    ...
    rounded_temps = np.round(temp_array).astype(int) // COLOR_TEMPERATURE_COARSENESS * COLOR_TEMPERATURE_COARSENESS
    ...
    for i, temp in enumerate(rounded_temps):
        if temp in lookup_table:
            ...
            rgb_array[i] = [0, 1, 0]  # Default color (red) if temperature is not in lookup table
...
if __name__ == "__main__":
    ...
    temps = np.linspace(1000,50000,1000000)
    lookup = load_color_lookup_table()
    start = time.time()
    process_temperatures(temps, lookup)
